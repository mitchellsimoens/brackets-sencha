...
define(function() {
    ...
    ClassManager.registerPreprocessor('inheritable statics', function(cls, config) {
        if (config.inheritableStatics) {
            Sencha.Util.apply(cls, config.inheritableStatics);
        }

        var superInheritable = cls.prototype.__getSuperMethod('inheritableStatics', cls.prototype.$superclass);

        if (superInheritable) {
            Sencha.Util.apply(cls, superInheritable);
        }
    });
